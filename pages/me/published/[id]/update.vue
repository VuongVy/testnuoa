  ...
  <div>
    <div class="card bg-base-100 max-w-2xl w-full mx-auto card-compact">
      <div class="px-5 py-3 border-b">
        <h3 class="card-title">Update PCF</h3>
      </div>
        ...
        <div class="form-control">
          ...
          <div v-for="(info, i) of additional" class="flex space-x-3 mb-3">
            <input
              ...
              type="text"
              v-model="additional[i].name"
            />
            <input
              class="input input-bordered w-full"
              type="text"
              v-model="additional[i].value"
            />
            <div class="grid place-content-center">
              <button
                class="btn btn-error btn-outline btn-circle btn-sm"
                @click="additional.splice(i, 1)"
              >
                X
              </button>
            </div>
      ...
      <div class="border-t card-actions p-5 justify-end space-x-5">
        <button class="btn btn-error btn-sm" @click="router.back()">
          Cancel
        </button>
...
const additional = reactive(new Array<{ name: string; value: string }>());

useBreadcrumb("Update PCF");

const update = async () => {
  const extra = additional.filter(({ name, value }) => name && value);
  ...
  const payload = {
    ...
    additionalData: Object.fromEntries(
      additional.map((it) => [it.name, it.value])
    ),
  };
...
onMounted(async () => {
  ...
  const extra = Object.entries(pcf.value.additionalData ?? {}).map(
